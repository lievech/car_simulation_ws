...
/*
 ...
 psi (psi)    = yaw angle or orientation
 v          = velocity
 delta          = actuator controls; steering angle
 ...
 x(t+dt) = x(t) + v(t) * cos(psi) * dt
 y(t+dt) = y(t) + v(t) * sin(psi) * dt

 orientation/yaw (psi):
 =====================

 psi(t+dt) = psi(t) + ​v(t)/L​f * delta * dt

 In a nutshell, we add a multiplicative factor of the steering angle, delta to psi. L​f measures the distance between the front of the vehicle and its center of gravity.
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars)
  {
    ...
    const double desired_velocity = 7;   // go speed racer, go!

    const double cost_func_cte_weight = 30;
    const double cost_func_epsi_weight = 30;
    const double cost_func_v_weight = 0.1;
    const double cost_func_steer_weight = 1;
    const double cost_func_throttle_weight = 0.1;         // penalize large throttle values (10x)
    const double cost_func_steer_rate_weight = 3000;     // penalize rapid steering angle changes (2000x).
...
void MPC_Controller::CalculatePolynomial(Eigen::VectorXd& localPathx ,Eigen::VectorXd& localPathy)
{
	wayPtPolynomialCoeffs = polyfit(localPathx, localPathy, 3);
	printf("wayCoeffs:%.2f , %.2f , %.2f , %.2f , size:%d\n"
			,wayPtPolynomialCoeffs(0)
			,wayPtPolynomialCoeffs(1)
			,wayPtPolynomialCoeffs(2)
			,wayPtPolynomialCoeffs(3)
			,wayPtPolynomialCoeffs.size());
}

Eigen::VectorXd MPC_Controller::polyfit(const Eigen::VectorXd& xvals, const Eigen::VectorXd& yvals, int order)
{
  ...
  const Eigen::HouseholderQR<Eigen::Matrix<double, -1, -1> > Q = A.householderQr();
  Eigen::VectorXd result= Q.solve(yvals);
  return result;
}
double MPC_Controller::polyeval(const Eigen::VectorXd& coeffs, double x)
{
	double result = 0.0;
	for (int i = 0; i < coeffs.size(); i++) {
		result += coeffs[i] * pow(x, i);
...
void MPC_Controller::CalculateErrors(const Eigen::VectorXd& current_state)
{
  ...
  //   psi(​t)     = orientation at time (t)
  //   ψdes(​t)  = desired orientation at time (t)
  // We already know psi(​t)​, because it’s part of our state. We don’t yet know ψdes(​t) (desired psi) - all we have so far is a polynomial to follow. ψdes(​t)
...
void MPC_Controller::Solve(const Eigen::VectorXd& current_state)
{
  /*
  ...
  */
  ...
  // Set px, py, psi, v, cte, epsi upper and lower bound values to double::min and double::max values respectively.
  ...
  {
    vars_lowerbound[i] = -0.536332;
    vars_upperbound[i] = 0.536332;
  }
  ...
  std::cout << "Cost " << cost << std::endl;
    ...
    for (int i = 0; i < N; i++) {
        ...
        std::vector<double> v(tmp, tmp + sizeof tmp / sizeof tmp[0]);
        predict_path_.push_back(v);
    }
  steering_angle_next = solution.x[steering_range_begin];
  throttle_next       = solution.x[throttle_range_begin];
  vel_next =  solution.x[v_range_begin+N-1];
}
}
